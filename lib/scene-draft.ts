...
function resolveScenePresetOverrideId(object: SceneObjectDraft, viewport: SceneViewport, breakpointIds: readonly SceneResponsiveBreakpointId[], preferredPresetId?: ScenePresetOverrideId) {
  ...
  if (configuredPresetIds.length === 0) return { presetId: undefined, usesNominalProfile: false };

  const exactPresetId = getScenePresetOverrideId(viewport);
  if (exactPresetId && object.presetOverrides[exactPresetId] !== undefined) return { presetId: exactPresetId, usesNominalProfile: false };
  if (preferredPresetId && preferredPresetId === exactPresetId && object.presetOverrides[preferredPresetId] !== undefined) return { presetId: preferredPresetId, usesNominalProfile: false };

  const sameWidth = configuredPresetIds.filter((id) => getScenePresetSize(id).width === viewport.width);
  if (sameWidth.length > 0) return { presetId: sameWidth.reduce((closest, id) => Math.abs(getScenePresetSize(id).height - viewport.height) < Math.abs(getScenePresetSize(closest).height - viewport.height) ? id : closest), usesNominalProfile: true };

  if (breakpointIds.some((breakpointId) => object.responsiveOverrides[breakpointId] !== undefined)) return { presetId: undefined, usesNominalProfile: false };
  ...
  if (sameBand.length === 0) return { presetId: undefined, usesNominalProfile: false };
