...
function resolveScenePresetOverrideId(object: SceneObjectDraft, viewport: SceneViewport, breakpointIds: readonly SceneResponsiveBreakpointId[], preferredPresetId?: ScenePresetOverrideId) {
  ...
  if (configuredPresetIds.length === 0) return { presetId: undefined, isApproximate: false, usesNominalProfile: false };

  const exactPresetId = getScenePresetOverrideId(viewport);
  if (exactPresetId && object.presetOverrides[exactPresetId] !== undefined) return { presetId: exactPresetId, isApproximate: false, usesNominalProfile: false };
  if (preferredPresetId && preferredPresetId === exactPresetId && object.presetOverrides[preferredPresetId] !== undefined) return { presetId: preferredPresetId, isApproximate: false, usesNominalProfile: false };

  const sameWidth = configuredPresetIds.filter((id) => getScenePresetSize(id).width === viewport.width);
  if (sameWidth.length > 0) return { presetId: sameWidth.reduce((closest, id) => Math.abs(getScenePresetSize(id).height - viewport.height) < Math.abs(getScenePresetSize(closest).height - viewport.height) ? id : closest), isApproximate: true, usesNominalProfile: true };

  if (breakpointIds.some((breakpointId) => object.responsiveOverrides[breakpointId] !== undefined)) return { presetId: undefined, isApproximate: false, usesNominalProfile: false };
  ...
  if (sameBand.length === 0) return { presetId: undefined, isApproximate: false, usesNominalProfile: false };
  ...
  }), isApproximate: true, usesNominalProfile: false };
}
function applySceneResponsiveOverrides(object: SceneObjectDraft, breakpointIds: readonly SceneResponsiveBreakpointId[]) { const resolved = { ...object }; for (const breakpointId of breakpointIds) Object.assign(resolved, object.responsiveOverrides[breakpointId]); return resolved; }
function applySceneResponsiveVisibility(object: SceneObjectDraft, resolved: SceneObjectDraft, breakpointIds: readonly SceneResponsiveBreakpointId[]) { for (const breakpointId of breakpointIds) { const visible = object.responsiveOverrides[breakpointId]?.visible; if (visible !== undefined) resolved.visible = visible; } }
export function resolveSceneObjectForViewport(object: SceneObjectDraft, viewport: SceneViewport, presetId?: ScenePresetOverrideId) { const breakpointIds = getSceneResponsiveBreakpoints(viewport); const presetResolution = resolveScenePresetOverrideId(object, viewport, breakpointIds, presetId); const resolved = presetResolution.usesNominalProfile && presetResolution.presetId ? applySceneResponsiveOverrides(object, getSceneResponsiveBreakpoints(getScenePresetSize(presetResolution.presetId))) : applySceneResponsiveOverrides(object, breakpointIds); if (presetResolution.presetId) { Object.assign(resolved, object.presetOverrides[presetResolution.presetId]); if (presetResolution.usesNominalProfile) applySceneResponsiveVisibility(object, resolved, breakpointIds); if (presetResolution.isApproximate) { const presetViewport = getScenePresetSize(presetResolution.presetId); if (resolved.anchorX === 'right') resolved.offsetX += presetViewport.width - viewport.width; if (resolved.anchorY === 'bottom') resolved.offsetY += presetViewport.height - viewport.height; } } return { breakpointId: breakpointIds.at(-1) ?? null, breakpointIds, presetId: presetResolution.presetId, object: resolved }; }
