...
const assets = [
  { out: 'halle-vendanges-automne.webp', commons: "Noël Hallé - Les Vendanges ou l'Automne - 1776.jpg", crop: true },
  { out: 'goya-vendimia-otono.webp', commons: 'Francisco de Goya - La vendimia o El Otoño (1786).jpg', crop: true },
  ...
  // Clean reproduction rather than the gallery-room photograph from the original shortlist.
  { out: 'levitan-golden-autumn.webp', commons: 'Levitan Zolotaya Osen.jpg' },
  // Clean museum reproduction; avoids the photographed/framed variant with reflections.
  ...
  { out: 'renoir-autumn-landscape.webp', commons: "Pierre-Auguste Renoir - Autumn Landscape (Paysage d'automne) - BF933 - Barnes Foundation.jpg", crop: true },
  ...
  { out: 'millais-autumn-leaves.webp', commons: 'John Everett Millais - Autumn Leaves.jpeg', crop: true },
  { out: 'grimshaw-october-gold.webp', commons: 'John Atkinson Grimshaw - October Gold.jpg', crop: true },
  ...
  { out: 'levitan-sokolniki.webp', commons: 'Levitan Sokolniki Autumn 1879.jpg', crop: true },
  { out: 'shishkin-autumn.webp', commons: 'Autumn 1892 (Shishkin).jpg', crop: true },
  ...
  { out: 'gifford-october-catskills.webp', commons: 'October in the Catskills by Sanford Robinson Gifford, 1879, High Museum of Art.jpg', crop: true },
  { out: 'richards-october.webp', commons: 'William Trost Richards, October, 1863, NGA 127263.jpg', crop: true },
];
...
async function commonsImageUrl(filename) {
  ...
  if (!info?.url) {
    throw new Error(`Wikimedia Commons file not resolved: ${title}`);
  }
...
async function buildAsset(asset) {
  ...
  await resized.webp({ quality: 84, effort: 6, smartSubsample: true }).toFile(outputPath);
  const after = await sharp(outputPath).metadata();
  console.log(`${asset.out}: ${before.width}x${before.height} -> ${after.width}x${after.height}${asset.crop ? ' [4:3 crop]' : ''}`);
}
