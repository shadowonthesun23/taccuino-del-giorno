...
export default function Home({ initialLang = 'IT' }: { initialLang?: LanguageCode }) {
    ...
    Promise.all([
      ...
      .then(([dati, operaData]) => {
        ...
        fetch(dataIso ? `/api/apod?data=${encodeURIComponent(dataIso)}` : '/api/apod')
          ...
          .then((apodData) => {
            if (requestId === latestDayRequestId) setApod(apodData);
          })
          .catch(() => {
            if (requestId === latestDayRequestId) setApod(null);
          })
          .finally(() => {
            if (requestId === latestDayRequestId) setApodLoading(false);
          });
  ...
  const cambiaLingua = useCallback(async (targetLang: LanguageCode) => {
    ...
    try {
      const testi = extractTranslatableText(dataOriginale);
      ...
      const tradotta = rebuildTranslatedData(dataOriginale, traduzioni);
  ...
  useEffect(() => {
    ...
    // The initial request must run once only; later date changes use the explicit controls.
  ...
  const toggleSavedDrawer = (trigger: HTMLButtonElement) => {
    setPopoverOpen(false);
    if (!savedDrawerOpen) {
      const rect = trigger.getBoundingClientRect();
      setSavedDrawerPos({
              ...
              <div className="flex flex-col gap-[3px] animate-menu-fade">
                ...
                {archivio.length > 0 && (
                  <button
                    ...
                    onClick={(event) => {
                      setMobileToolsOpen(false);
                      toggleArchive(mobileToolsTriggerRef.current ?? event.currentTarget);
                    }}
                    aria-expanded={popoverOpen}
                    aria-haspopup="dialog"
                  >
