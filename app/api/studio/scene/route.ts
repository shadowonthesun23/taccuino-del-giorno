...
import { HOME_SCENE_DRAFT_BASELINE_V1, resolveSceneDraftStrict } from '@/lib/scene-draft-editor';
...
export async function PUT(request: Request) {
  ...
  if (!validation || !validation.ok) return NextResponse.json({ error: 'Scena non valida.', issues: validation?.issues }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  let validation = action === 'rollback' ? null : validateSceneDraft(body);
  if (action === 'rollback') {
    ...
    const sourceScene = resolveSceneDraftStrict(source.data.scene);
    if (!sourceScene) return NextResponse.json({ error: 'Snapshot sorgente non valida.' }, { status: 400 });
    validation = { ok: true, value: sourceScene };
    sourceVersionId = source.data.id;
    label = `Ripristino di v${source.data.version_number}`;
  }
  if (!validation) return NextResponse.json({ error: 'Scena non valida.' }, { status: 400 });
